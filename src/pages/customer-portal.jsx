...
import { useCustomerProfile, useProductCatalog, useValidateDiscount, useCustomerOrders, usePlaceOrder, useCustomerInvoices, usePayInvoice, useCustomerPaymentSettings, useUploadPaymentProof, } from "@/hooks/use-customer-portal";
...
import { ShoppingCart, Package, FileText, Minus, Plus, Trash2, CheckCircle2, Truck, Star, Image, Info, ChevronDown, ChevronUp, ShoppingBag, AlertTriangle, Printer, Download, } from "lucide-react";
...
import { usePublicQcRemarks } from "@/hooks/use-production";
/* ─── Shared helpers ──────────────────────────────────────────────────────── */
...
const INVOICE_STATUS_COLORS = {
    ...
    sales_verified: "bg-cyan-100 text-cyan-700",
...
function BrowseTab({ onOrderPlaced }) {
    ...
    return (_jsxs("div", { className: "space-y-10 pb-24", children: [storefrontLoading && !storefront && _jsx("div", { className: "grid grid-cols-2 gap-3 md:grid-cols-4", children: [1, 2, 3, 4, 5, 6, 7, 8].map(i => _jsx(Skeleton, { className: "aspect-square rounded-lg" }, i)) }), colls.length > 0 && _jsxs("section", { className: "space-y-4", "aria-labelledby": "collection-list-heading", children: [_jsx("h2", { id: "collection-list-heading", className: "text-center text-lg font-semibold uppercase tracking-[0.25em] text-emerald-950 dark:text-emerald-50", children: "Collection list" }), _jsx("div", { className: "grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-4", children: colls.map(c => {
                ...
                }) }))] }), cart.length > 0 && (_jsxs("div", { className: "fixed inset-x-0 bottom-0 z-40 border-t bg-background/95 p-3 shadow-[0_-8px_20px_rgba(0,0,0,0.08)] backdrop-blur sm:px-6", children: [_jsxs("div", { className: "mx-auto flex w-full max-w-5xl items-center justify-between gap-3", children: [_jsxs("div", { className: "min-w-0", children: [_jsxs("p", { className: "text-sm font-semibold", children: [cart.length, " item", cart.length !== 1 ? "s" : "", " ready"] }), _jsxs("p", { className: "text-xs text-muted-foreground", children: ["Estimated total: ", fmt(total)] })] }), _jsxs(Button, { className: "shrink-0", onClick: () => setShowCheckout(true), children: [_jsx(ShoppingCart, { className: "mr-1.5 h-4 w-4" }), "Checkout"] })] })] })), _jsx(BrowseCheckoutDialog, { open: showCheckout, onOpenChange: setShowCheckout, handleSubmit, onCheckout, cart, changeQty, subtotal, appliedDiscount, discountInput, setDiscountInput, applyDiscount, register, errors, checkoutSubmitDisabled, placeOrderButtonLabel, setShowCheckout, total, fmt, watch }), _jsx(OrderInvoiceDialog, { open: showOrderInvoice, onOpenChange: handleInvoiceDialogChange, order: confirmedOrder })] }));
}
/* ═══════════════════════════════════════════════════════════════════════════ */
/*  TAB 2 — MY ORDERS                                                          */
/* ═══════════════════════════════════════════════════════════════════════════ */
...
function CustomerQcFeed({ taskId }) {
    const { data: remarks = [], isLoading } = usePublicQcRemarks(taskId);
    if (!taskId)
        return null;
    if (isLoading)
        return _jsx(Skeleton, { className: "h-20 w-full" });
    if (remarks.length === 0)
        return _jsx("p", { className: "text-xs text-muted-foreground", children: "No production manager remarks shared yet." });
    return (_jsx("div", { className: "space-y-2 max-h-44 overflow-y-auto pr-1", children: remarks.slice(0, 8).map((r) => (_jsxs("div", { className: "rounded-lg border bg-card p-2.5 text-xs", children: [_jsxs("div", { className: "mb-1 flex flex-wrap items-center gap-1.5", children: [r.result && _jsx(Badge, { className: "text-[10px] bg-indigo-100 text-indigo-700", children: String(r.result).toUpperCase() }), _jsx("span", { className: "text-muted-foreground", children: r.inspectorName ?? "Production manager" })] }), _jsx("p", { children: r.remarks ?? r.remark ?? "Update posted." }), r.createdAt && _jsx("p", { className: "mt-1 text-[10px] text-muted-foreground", children: new Date(r.createdAt).toLocaleString() })] }, r.id))) }));
}
function MyOrdersTab() {
    ...
    useEffect(() => {
        ...
        const focusOrderId = Number(params.get("focusOrderId") || 0);
        if (focusOrderId > 0)
...
function InvoicesTab() {
    ...
    const { data: paymentSettings } = useCustomerPaymentSettings();
    ...
    const onPay = async (data) => {
        if (!payingId)
            return;
        if (data.paymentMethod === "JazzCash" && !data.paymentReference && paymentSettings?.jazzcash?.accountNumber) {
            toast({
                variant: "destructive",
                title: "Reference required",
                description: "Please enter JazzCash transaction reference before confirming.",
            });
            return;
        }
        if (!proofFile) {
            toast({
                ...
                description: "Please attach screenshot/receipt so Sales/Accounts can verify your payment.",
            });
            return;
        }
        try {
            await uploadPaymentProof.mutateAsync({ id: payingId, file: proofFile });
            ...
            toast({ title: "Submitted for verification", description: "Sales/Accounts will verify your payment proof before marking paid." });
            setPayingId(null);
            setProofFile(null);
            reset();
        }
    ...
    return (_jsxs("div", { className: "space-y-4", children: [outstanding > 0 && (_jsxs(Alert, { className: "border-amber-200 bg-amber-50 dark:bg-amber-950/20", children: [_jsx(AlertTriangle, { className: "h-4 w-4 text-amber-600" }), _jsxs(AlertDescription, { children: ["You have ", _jsx("strong", { children: fmt(outstanding) }), " outstanding balance across unpaid invoices."] })] })), isLoading ? (_jsx("div", { className: "space-y-3", children: [1, 2].map(i => _jsx(Skeleton, { className: "h-24 w-full" }, i)) })) : invoices.length === 0 ? (_jsxs("div", { className: "flex flex-col items-center justify-center rounded-xl border bg-card py-14 text-muted-foreground", children: [_jsx(FileText, { className: "mb-3 h-10 w-10" }), _jsx("p", { children: "No invoices yet" }), _jsx("p", { className: "text-xs mt-1", children: "Invoices will appear here once generated by our team" })] })) : (_jsx("div", { className: "space-y-3", children: invoices.map(inv => {
                    ...
                    return (_jsx(Card, { className: cn("overflow-hidden", isOverdue && "border-red-200"), children: _jsx(CardContent, { className: "p-4", children: _jsxs("div", { className: "flex flex-col gap-3 sm:flex-row sm:items-center", children: [_jsxs("div", { className: "flex-1 min-w-0", children: [_jsxs("div", { className: "flex flex-wrap items-center gap-2 mb-1", children: [_jsx("span", { className: "font-mono text-sm font-bold", children: inv.invoiceNumber }), _jsx(Badge, { className: cn("text-[11px]", statusColor), children: INVOICE_STATUS_LABELS[inv.status] ?? inv.status }), isOverdue && _jsx(Badge, { className: "bg-red-100 text-red-700 text-[11px]", children: "Overdue" })] }), _jsxs("p", { className: "text-sm", children: [inv.dueDate && _jsxs("span", { className: "text-muted-foreground", children: ["Due ", new Date(inv.dueDate).toLocaleDateString()] }), inv.paidAt && _jsxs("span", { className: "ml-2 text-green-600 flex-inline items-center gap-1", children: [_jsx(CheckCircle2, { className: "h-3 w-3 inline" }), " Paid ", new Date(inv.paidAt).toLocaleDateString()] })] }), inv.status === "pending_verification" && (_jsx("p", { className: "mt-1 text-xs text-amber-700", children: "Proof submitted. Waiting for Sales verification." })), inv.status === "sales_verified" && (_jsx("p", { className: "mt-1 text-xs text-cyan-700", children: "Sales verified. Waiting for Accounts final posting." })), inv.paymentMethod && _jsxs("p", { className: "mt-0.5 text-xs text-muted-foreground", children: ["Via ", inv.paymentMethod, inv.paymentReference && ` · Ref: ${inv.paymentReference}`] }), inv.pdfUrl && _jsx("a", { href: resolvePublicAssetUrl(inv.pdfUrl), target: "_blank", rel: "noopener noreferrer", className: "mt-2 inline-flex items-center gap-1.5 text-sm font-medium text-emerald-700 underline-offset-2 hover:underline", children: [_jsx(Download, { className: "h-4 w-4 shrink-0", "aria-hidden": true }), "Invoice PDF (from sales)"] })] }), _jsxs("div", { className: "flex items-center gap-4", children: [_jsxs("div", { className: "text-right", children: [_jsx("p", { className: "text-xl font-bold tabular-nums", children: fmt(inv.totalAmount) }), inv.discountAmount > 0 && _jsxs("p", { className: "text-xs text-green-600", children: ["Incl. ", fmt(inv.discountAmount), " discount"] })] }), inv.status !== "paid" && inv.status !== "cancelled" && inv.status !== "pending_verification" && inv.status !== "sales_verified" && (_jsx(Button, { onClick: () => setPayingId(inv.id), className: "shrink-0", children: "Submit payment proof" }))] })] }) }) }, inv.id));
                }) })), _jsx(Dialog, { open: !!payingId, onOpenChange: v => { if (!v) {
                    setPayingId(null);
                    setProofFile(null);
                    reset();
                } }, children: _jsxs(DialogContent, { children: [_jsx(DialogHeader, { children: _jsx(DialogTitle, { children: "Record payment" }) }), _jsxs("form", { onSubmit: handleSubmit(onPay), className: "space-y-4", children: [_jsxs(Alert, { children: [_jsx(Info, { className: "h-4 w-4" }), _jsx(AlertDescription, { className: "text-sm", children: "Please complete payment first, then submit proof. Sales/Accounts will verify before marking invoice paid." })] }), _jsxs("div", { className: "space-y-1", children: [_jsx(Label, { children: "Payment method *" }), _jsxs("select", { className: "flex h-9 w-full rounded-md border border-input bg-background px-3 py-1 text-sm shadow-sm focus-visible:outline-none focus-visible:ring-1", ...register("paymentMethod", { required: true }), children: [_jsx("option", { value: "", children: "Select method\u2026" }), _jsx("option", { children: "Bank Transfer" }), _jsx("option", { children: "Credit Card" }), _jsx("option", { children: "Debit Card" }), _jsx("option", { children: "Mobile Payment" }), _jsx("option", { children: "Cash" }), _jsx("option", { children: "Cheque" })] })] }), _jsxs("div", { className: "space-y-1", children: [_jsx(Label, { children: "Transaction reference / receipt no." }), _jsx(Input, { ...register("paymentReference"), placeholder: "e.g. TXN-20240101-001" })] }), _jsxs("div", { className: "space-y-1", children: [_jsx(Label, { children: "Payment screenshot / receipt *" }), _jsx(Input, { type: "file", accept: "image/*,application/pdf", onChange: (e) => setProofFile(e.target.files?.[0] ?? null) }), _jsx("p", { className: "text-xs text-muted-foreground", children: "Required for verification by Sales Manager / Accounts / Admin." })] }), _jsxs(DialogFooter, { children: [_jsx(Button, { variant: "outline", type: "button", onClick: () => { setPayingId(null); setProofFile(null); reset(); }, children: "Cancel" }), _jsx(Button, { type: "submit", disabled: payInvoice.isPending || uploadPaymentProof.isPending, children: "Submit for verification" })] })] })] }) })] }));
}
/* ═══════════════════════════════════════════════════════════════════════════ */
/*  MAIN PAGE                                                                  */
/* ═══════════════════════════════════════════════════════════════════════════ */
export default function CustomerPortalPage({ initialTab = "browse", hideTabs = false }) {
    ...
    useEffect(() => {
        ...
        if (t && ["browse", "orders", "invoices"].includes(t))
            setActiveTab(t);
    }, [location]);
