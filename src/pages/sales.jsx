...
import { useSalesOverview, useSalesOrders, useCreateSalesOrder, useUpdateSalesOrder, useAddOrderUpdateWithImages, useSalesInvoices, useGenerateInvoice, useUpdateInvoice, useUploadInvoicePdf, useSalesDiscounts, useCreateDiscount, useUpdateDiscount, useDeleteDiscount, useSalesReceivables, } from "@/hooks/use-sales-manager";
...
function OrdersTab() {
    ...
    const addUpdate = useAddOrderUpdateWithImages();
    const [search, setSearch] = useState("");
    const [statusF, setStatusF] = useState("all");
    const [showCreate, setShowCreate] = useState(false);
    const [expandedId, setExpandedId] = useState(null);
    const [updateOrderId, setUpdateOrderId] = useState(null);
    const [orderImagePanelId, setOrderImagePanelId] = useState(null);
...
function InvoicesTab() {
    ...
    return (_jsxs("div", { className: "space-y-4 min-w-0", children: [_jsxs("div", { className: "flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center", children: [_jsxs(Select, { value: statusF, onValueChange: setStatusF, children: [_jsx(SelectTrigger, { className: "w-full sm:w-36", children: _jsx(SelectValue, {}) }), _jsxs(SelectContent, { children: [_jsx(SelectItem, { value: "all", children: "All" }), Object.entries(INVOICE_STATUS_CONFIG).map(([k, v]) => _jsx(SelectItem, { value: k, children: v.label }, k))] })] }), _jsxs(Button, { className: "w-full sm:ml-auto sm:w-auto", onClick: () => setShowGen(true), children: [_jsx(Plus, { className: "mr-1.5 h-4 w-4" }), "Generate invoice"] })] }), isLoading ? _jsx(Skeleton, { className: "h-48 w-full" }) : (_jsx(Card, { children: _jsx(CardContent, { className: "p-0", children: _jsx("div", { className: "overflow-x-auto", children: _jsxs(Table, { children: [_jsx(TableHeader, { children: _jsxs(TableRow, { children: [_jsx(TableHead, { children: "Invoice #" }), _jsx(TableHead, { children: "Customer" }), _jsx(TableHead, { className: "text-right", children: "Amount" }), _jsx(TableHead, { children: "Due Date" }), _jsx(TableHead, { children: "Status" }), _jsx(TableHead, { children: "Actions" })] }) }), _jsx(TableBody, { children: filtered.map(inv => {
                                    ...
                                    return (_jsxs(TableRow, { children: [_jsx(TableCell, { className: "font-mono text-xs", children: inv.invoiceNumber }), _jsxs(TableCell, { children: [_jsx("p", { className: "font-medium", children: inv.customerName }), _jsx("p", { className: "text-xs text-muted-foreground", children: inv.customerEmail }), inv.paymentMethod && _jsxs("p", { className: "text-xs text-muted-foreground", children: ["Via ", inv.paymentMethod, inv.paymentReference ? ` · Ref: ${inv.paymentReference}` : ""] }), inv.paymentProofUrl && _jsx("a", { href: resolvePublicAssetUrl(inv.paymentProofUrl), target: "_blank", rel: "noreferrer", className: "text-xs text-primary hover:underline", children: "View payment proof" })] }), _jsx(TableCell, { className: "text-right font-semibold tabular-nums", children: fmt(inv.totalAmount) }), _jsxs(TableCell, { className: cn("text-sm", isOverdue && "font-semibold text-red-600"), children: [inv.dueDate ? new Date(inv.dueDate).toLocaleDateString() : "—", isOverdue && " (Overdue)"] }), _jsx(TableCell, { children: _jsx(Badge, { className: cn("text-[11px]", cfg.color), children: cfg.label }) }), _jsx(TableCell, { children: _jsxs("div", { className: "flex flex-wrap gap-1", children: [inv.status === "pending_verification" && canSalesVerify && (_jsxs(Fragment, { children: [_jsxs(Button, { size: "sm", variant: "outline", className: "h-7 text-xs", onClick: () => salesVerify(inv), children: [_jsx(CheckCircle2, { className: "mr-1 h-3.5 w-3.5" }), "Sales verify"] }), _jsx(Button, { size: "sm", variant: "outline", className: "h-7 text-xs", onClick: () => rejectPayment(inv), children: "Reject / Request re-submit" })] })), inv.status === "sales_verified" && canAccountsPost && (_jsxs(Button, { size: "sm", variant: "outline", className: "h-7 text-xs", onClick: () => accountMarkPaid(inv), children: [_jsx(CheckCircle2, { className: "mr-1 h-3.5 w-3.5" }), "Accounts mark paid"] })), inv.status === "draft" && (_jsx(Button, { size: "sm", variant: "outline", className: "h-7 text-xs", onClick: () => updateInvoice.mutateAsync({ id: inv.id, status: "sent" }).then(() => toast({ title: "Invoice sent" })).catch((e) => toast({ variant: "destructive", title: "Error", description: e.message })), children: "Send" })), _jsxs(Button, { size: "sm", variant: "outline", className: "h-7 text-xs", onClick: () => uploadPdf(inv), children: [_jsx(Upload, { className: "mr-1 h-3.5 w-3.5" }), "Upload PDF"] }), inv.pdfUrl && (_jsx("a", { href: resolvePublicAssetUrl(inv.pdfUrl), target: "_blank", rel: "noreferrer", className: "inline-flex h-7 items-center rounded-md border px-2 text-xs hover:bg-muted", children: "Open PDF" }))] }) })] }, inv.id));
