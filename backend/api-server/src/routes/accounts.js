...
/* ── CSV helpers ───────────────────────────────────────────────────────────── */
const CSV_COLUMNS = ["code", "name", "type", "subtype", "normal_balance", "description"];
...
/* ── GET /accounts ─────────────────────────────────────────────────────────── */
router.get("/accounts", authenticate, async (_req, res, next) => {
    try {
        const rows = await db
            .select()
...
router.get("/accounts/sample.csv", authenticate, async (_req, res) => {
    const sample = [
...
router.get("/accounts/export.csv", authenticate, async (_req, res, next) => {
    try {
...
router.post("/accounts/import", authenticate, requireRole("admin", "accountant"), async (req, res, next) => {
    try {
        const { csv } = req.body;
        ...
        const iSub = idx("subtype");
        const iNB = idx("normal_balance");
        ...
        for (let i = 1; i < rawLines.length; i++) {
            ...
            if (!code || !name) {
                errors.push(`Row ${i + 1}: code and name are required.`);
                continue;
            }
            ...
            if (!VALID_NB.includes(normalBalance)) {
                errors.push(`Row ${i + 1}: invalid normal_balance "${normalBalance}".`);
                continue;
            }
            try {
...
/* ── POST /accounts/seed ─────────────────────────────────────────────────────*/
router.post("/accounts/seed", authenticate, requireRole("admin"), async (req, res, next) => {
    try {
        ...
        const inserted = await db.insert(chartOfAccountsTable).values(STANDARD_ACCOUNTS.map((a) => ({ ...a, isActive: true }))).returning({ id: chartOfAccountsTable.id });
...
/* ── POST /accounts ──────────────────────────────────────────────────────────*/
router.post("/accounts", authenticate, requireRole("admin", "accountant"), async (req, res, next) => {
    ...
    try {
        ...
        if (err.code === "23505") {
            res.status(409).json({ error: "DUPLICATE_CODE", message: `Account code ${code} already exists.` });
            return;
        }
        next(err);
    }
...
/* ── PATCH /accounts/:id ─────────────────────────────────────────────────────*/
router.patch("/accounts/:id", authenticate, requireRole("admin", "accountant"), async (req, res, next) => {
...
router.delete("/accounts/:id", authenticate, requireRole("admin"), async (req, res, next) => {
    const id = parseInt(req.params.id, 10);
    if (isNaN(id)) {
        res.status(400).json({ error: "INVALID_ID" });
        return;
