...
async function getAzureADToken() {
    ...
    const resp = await fetch(`https://login.microsoftonline.com/${POWERBI_TENANT_ID}/oauth2/v2.0/token`, {
        method: "POST",
    ...
    if (!resp.ok)
        throw new Error(`Azure AD token failed (${resp.status}): ${await resp.text()}`);
...
async function generateEmbedToken(workspaceId, reportId) {
    ...
    const resp = await fetch(`https://api.powerbi.com/v1.0/myorg/groups/${workspaceId}/reports/${reportId}/GenerateToken`, {
        method: "POST",
...
async function getReportEnv(id) {
...
/* ════════════════════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════════════════════ */
router.get("/powerbi/reports", authenticate, async (req, res) => {
    if (!req.user) {
...
/* ════════════════════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════════════════════ */
router.post("/powerbi/embed-token", authenticate, async (req, res) => {
    if (!req.user) {
        res.status(401).json({ error: "Unauthenticated" });
        return;
    }
...
/* ════════════════════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════════════════════ */
async function withModuleAccess(req, res, module, handler) {
    if (!req.user) {
        res.status(401).json({ error: "Unauthenticated" });
        return;
    }
...
/* ── Supplier Ledger ─────────────────────────────────────────────────────────── */
router.get("/powerbi/data/supplier-ledger", authenticate, async (req, res) => {
    await withModuleAccess(req, res, "accounting", async () => {
        const result = await cachedQuery("supplier-ledger", () => db.execute(sql `
...
/* ── Expense vs Income ───────────────────────────────────────────────────────── */
router.get("/powerbi/data/expense-income", authenticate, async (req, res) => {
    await withModuleAccess(req, res, "accounting", async () => {
        const result = await cachedQuery("expense-income", () => db.execute(sql `
...
/* ── Payroll Summary ─────────────────────────────────────────────────────────── */
router.get("/powerbi/data/payroll-summary", authenticate, async (req, res) => {
    await withModuleAccess(req, res, "payroll", async () => {
        const result = await cachedQuery("payroll-summary", () => db.execute(sql `
...
/* ── Profit Margin ───────────────────────────────────────────────────────────── */
router.get("/powerbi/data/profit-margin", authenticate, async (req, res) => {
    await withModuleAccess(req, res, "accounting", async () => {
        const [monthly, inventory] = await Promise.all([
            cachedQuery("profit-margin:monthly", () => db.execute(sql `
          SELECT DATE_TRUNC('month', transaction_date)::date AS month,
        ...
        `)),
            cachedQuery("profit-margin:inventory", () => db.execute(sql `
        ...
        res.json({ monthly: monthly.rows, inventoryValue: inventory.rows });
    });
});
/* ── Trial Balance ───────────────────────────────────────────────────────────── */
router.get("/powerbi/data/trial-balance", authenticate, async (req, res) => {
    await withModuleAccess(req, res, "accounting", async () => {
        const result = await cachedQuery("trial-balance", () => db.execute(sql `
...
/* ── Inventory Analysis ──────────────────────────────────────────────────────── */
router.get("/powerbi/data/inventory-analysis", authenticate, async (req, res) => {
    await withModuleAccess(req, res, "inventory", async () => {
        const result = await cachedQuery("inventory-analysis", () => db.execute(sql `
      ...
      `));
        ...
        const summary = {
            ...
            byType: Object.fromEntries([...new Set(rows.map((r) => r.type))].map((t) => [
                t,
...
/* ── HR Dashboard ────────────────────────────────────────────────────────────── */
router.get("/powerbi/data/hr-dashboard", authenticate, async (req, res) => {
    try {
        await withModuleAccess(req, res, "hr", async () => {
            const [employees, attendance] = await Promise.all([
                cachedQuery("hr-dashboard:employees", () => db.execute(sql `
        ...
        `)),
                cachedQuery("hr-dashboard:attendance", () => db.execute(sql `
          SELECT e.department,
        ...
        `)),
            ...
            let reviewRows = [];
            try {
                const reviews = await cachedQuery("hr-dashboard:reviews", () => db.execute(sql `
          SELECT e.department,
        ...
        `));
            ...
            const deptBreakdown = [...new Set(empRows.map((e) => e.department))].map((dept) => {
                ...
                const att = attRows.find((r) => r.department === dept) ?? {};
                const rev = reviewRows.find((r) => r.department === dept) ?? {};
        ...
        res.status(500).json({ error: "HR_DASHBOARD_ERROR", message: msg });
    }
});
/* ── Sales Overview ──────────────────────────────────────────────────────────── */
router.get("/powerbi/data/sales-overview", authenticate, async (req, res) => {
    await withModuleAccess(req, res, "sales", async () => {
        const result = await cachedQuery("sales-overview", () => db.execute(sql `
        SELECT DATE_TRUNC('month', transaction_date)::date AS month,
               COUNT(*)::int AS transaction_count,
               SUM(CASE WHEN type='income'  THEN amount ELSE 0 END)::float AS revenue,
               SUM(CASE WHEN type='expense' THEN amount ELSE 0 END)::float AS expenses,
