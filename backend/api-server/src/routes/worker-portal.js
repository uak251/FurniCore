...
import { z } from "zod";
import { db, usersTable, employeesTable, manufacturingTasksTable, attendanceTable, payrollTable, productsTable, } from "@workspace/db";
...
async function findEmployeeForUser(userId) {
    const [emp] = await db
        .select()
        .from(employeesTable)
...
/*  PROFILE                                                                    */
/* ═══════════════════════════════════════════════════════════════════════════ */
router.get("/worker-portal/me", ...workerOnly, async (req, res) => {
...
/* ═══════════════════════════════════════════════════════════════════════════ */
router.get("/worker-portal/tasks", ...workerOnly, async (req, res) => {
    const userId = req.user.id;
...
router.patch("/worker-portal/tasks/:id", ...workerOnly, async (req, res) => {
    ...
    if (parsed.data.progress !== undefined)
        patch.progress = parsed.data.progress;
    if (parsed.data.actualHours !== undefined)
        patch.actualHours = String(parsed.data.actualHours);
...
/* ═══════════════════════════════════════════════════════════════════════════ */
router.get("/worker-portal/attendance", ...workerOnly, async (req, res) => {
    const userId = req.user.id;
    const emp = await findEmployeeForUser(userId);
    if (!emp) {
...
/* ═══════════════════════════════════════════════════════════════════════════ */
router.get("/worker-portal/payroll", ...workerOnly, async (req, res) => {
    const userId = req.user.id;
    const emp = await findEmployeeForUser(userId);
    if (!emp) {
