...
import { db, inventoryTable, productsTable, employeesTable, payrollTable, } from "@workspace/db";
...
router.get("/bulk/inventory/export", ...invRole, async (_req, res, next) => {
    try {
...
router.get("/bulk/products/export", ...invRole, async (_req, res, next) => {
    try {
...
router.get("/bulk/employees/export", ...hrRole, async (_req, res, next) => {
    try {
...
router.post("/bulk/payroll/import", ...payRole, csvBody, async (req, res, next) => {
    try {
        const { rows } = parseCsv(req.body);
        const result = emptyResult();
        /* Cache employee lookups */
        const empCache = {};
        const lookupEmployee = async (email) => {
            if (email in empCache)
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            const [existing] = await db
                ...
                .from(payrollTable)
                .where(and(eq(payrollTable.employeeId, employeeId), eq(payrollTable.month, d.month), eq(payrollTable.year, d.year)));
            if (existing) {
...
router.get("/bulk/payroll/export", ...payRole, async (_req, res, next) => {
    try {
