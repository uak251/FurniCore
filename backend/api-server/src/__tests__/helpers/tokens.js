...
export function rolesExcept(...exclude) {
 ...
 */
export function makeToken(role, userId = 1, email = `${role}@test.com`) {
    return jwt.sign({ id: userId, email, role }, ACCESS_SECRET, {
        expiresIn: "1h",
    });
}
/** Pre-built token for each role (userId=1 for each) */
export const tokens = Object.fromEntries(ALL_ROLES.map((r) => [r, makeToken(r)]));
