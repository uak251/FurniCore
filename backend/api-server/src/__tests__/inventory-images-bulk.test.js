...
describe("POST /api/images/inventory/:id/bulk", () => {
    ...
    it.each([
    ...
    ])("accepts %s and persists rows via db.insert…values (mime %s)", async (mime, filename) => {
        const token = makeToken("admin");
        const captured = [];
        vi.mocked(db.insert).mockImplementationOnce(() => ({
            values: (rows) => {
                ...
                captured.push(...arr);
                return {
                    returning: vi.fn().mockResolvedValue(arr.map((row, i) => ({
                        id: i + 1,
                        ...row,
                    }))),
                };
        ...
        for (const role of ["admin", "inventory_manager"]) {
            vi.mocked(db.insert).mockImplementationOnce(() => ({
                values: (rows) => {
                    const arr = Array.isArray(rows) ? rows : [rows];
                    return {
                        returning: vi.fn().mockResolvedValue(arr.map((_, i) => ({ id: i + 1 }))),
                    };
                },
            }));
            const res = await request(app)
