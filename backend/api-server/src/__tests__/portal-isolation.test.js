...
vi.mock("@workspace/db", () => {
    ...
    for (const m of chainMethods)
        chain[m] = () => chain;
    chain.then = (res) => Promise.resolve([]).then(res);
...
import { tokens, rolesExcept } from "./helpers/tokens";
...
const bearer = (role) => `Bearer ${tokens[role]}`;
/* ═══════════════════════════════════════════════════════════════════════════
   Shared test builder
   ═══════════════════════════════════════════════════════════════════════════ */
/**
 ...
 */
function accessMatrix(method, path, allowedRoles, body) {
    ...
    it.each(deniedRoles)("returns 403 for role '%s' (not in allowed list)", async (role) => {
        const res = await request[method](`/api${path}`)
            .set("Authorization", bearer(role))
            .send(body ?? {});
        expect(res.status).toBe(403);
    });
    it.each(allowedRoles)("passes auth (not 401/403) for allowed role '%s'", async (role) => {
        const res = await request[method](`/api${path}`)
            .set("Authorization", bearer(role))
            .send(body ?? {});
