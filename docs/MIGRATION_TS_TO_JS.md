# TypeScript → JavaScript migration (FurniCore apps)

This document describes how the `backend/api-server` and `frontend/furnicore` packages were moved from TypeScript to JavaScript, and how to repeat or extend the process.

## Scope

| Area | Status |
|------|--------|
| `backend/api-server` | Converted: `src/**/*.ts` → `.js` |
| `frontend/furnicore` | Converted: `src/**/*.{ts,tsx}` → `.{js,jsx}`, configs `vite.config.js`, `vitest.config.js` |
...
5. **Remove TS project files** — Delete `tsconfig.json` in those app packages; add `jsconfig.json` for editor path aliases (`@/*`) where helpful.
...
9. **Verify** — `pnpm -r --filter "./backend/api-server" --filter "./frontend/furnicore" run build` and `run test`.
...
```bash
node scripts/ts-to-js-convert.mjs backend/api-server
node scripts/ts-to-js-convert.mjs frontend/furnicore
```
...
- Root: `eslint.config.js` — `recommended` for Node (`backend/api-server/src`) and React (`frontend/furnicore/src`).
...
- Convert `lib/*` to JavaScript or keep TS and treat `frontend/` / `backend/` app packages as the JS application layer.
