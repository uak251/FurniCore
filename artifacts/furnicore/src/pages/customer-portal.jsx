...
import { ShoppingCart, Package, FileText, Minus, Plus, Trash2, CheckCircle2, Truck, Star, Image, Info, ChevronDown, ChevronUp, ShoppingBag, Tag, AlertTriangle, } from "lucide-react";
...
function BrowseTab({ cart, setCart, onOrderPlaced, }) {
    ...
    return (_jsxs("div", { className: "space-y-4", children: [_jsxs("div", { className: "flex flex-wrap items-center gap-3", children: [_jsx(Input, { className: "w-56", placeholder: "Search products\u2026", value: search, onChange: e => setSearch(e.target.value) }), _jsxs(Select, { value: categoryF, onValueChange: setCategoryF, children: [_jsx(SelectTrigger, { className: "w-40", children: _jsx(SelectValue, { placeholder: "Category" }) }), _jsxs(SelectContent, { children: [_jsx(SelectItem, { value: "all", children: "All categories" }), categories.map(c => _jsx(SelectItem, { value: c, children: c }, c))] })] }), cart.length > 0 && (_jsxs(Button, { className: "ml-auto", onClick: () => setShowCheckout(true), children: [_jsx(ShoppingCart, { className: "mr-1.5 h-4 w-4" }), "Checkout (", cart.length, ") \u2014 ", fmt(total)] }))] }), isLoading ? (_jsx("div", { className: "grid gap-4 sm:grid-cols-2 lg:grid-cols-3", children: [1, 2, 3, 4, 5, 6].map(i => _jsx(Skeleton, { className: "h-48 rounded-xl" }, i)) })) : filtered.length === 0 ? (_jsxs("div", { className: "flex flex-col items-center justify-center py-16 text-muted-foreground", children: [_jsx(Package, { className: "mb-3 h-10 w-10" }), _jsx("p", { children: "No products match your search" })] })) : (_jsx("div", { className: "grid gap-4 sm:grid-cols-2 lg:grid-cols-3", children: filtered.map(product => {
                ...
                }) })), _jsx(Dialog, { open: showCheckout, onOpenChange: setShowCheckout, children: _jsxs(DialogContent, { className: "max-h-[90vh] max-w-lg overflow-y-auto", children: [_jsx(DialogHeader, { children: _jsxs(DialogTitle, { className: "flex items-center gap-2", children: [_jsx(ShoppingCart, { className: "h-5 w-5" }), "Checkout"] }) }), _jsxs("form", { onSubmit: handleSubmit(onCheckout), className: "space-y-5", children: [_jsxs("div", { className: "rounded-lg border divide-y text-sm", children: [cart.map(item => (_jsxs("div", { className: "flex items-center justify-between px-3 py-2 gap-3", children: [_jsxs("div", { className: "flex-1 min-w-0", children: [_jsx("p", { className: "truncate font-medium", children: item.product.name }), _jsxs("p", { className: "text-xs text-muted-foreground", children: [fmt(item.product.sellingPrice), " each"] })] }), _jsxs("div", { className: "flex items-center gap-1.5 shrink-0", children: [_jsx(Button, { type: "button", size: "icon", variant: "outline", className: "h-6 w-6", onClick: () => changeQty(item.product.id, -1), children: _jsx(Minus, { className: "h-3 w-3" }) }), _jsx("span", { className: "w-6 text-center tabular-nums", children: item.quantity }), _jsx(Button, { type: "button", size: "icon", variant: "outline", className: "h-6 w-6", onClick: () => changeQty(item.product.id, 1), children: _jsx(Plus, { className: "h-3 w-3" }) })] }), _jsx("span", { className: "w-20 text-right font-semibold tabular-nums shrink-0", children: fmt(item.product.sellingPrice * item.quantity) })] }, item.product.id))), _jsxs("div", { className: "flex justify-between px-3 py-2 font-medium", children: [_jsx("span", { children: "Subtotal" }), _jsx("span", { className: "font-mono", children: fmt(subtotal) })] }), appliedDiscount && _jsxs("div", { className: "flex justify-between px-3 py-1 text-green-700", children: [_jsxs("span", { className: "flex items-center gap-1", children: [_jsx(Tag, { className: "h-3.5 w-3.5" }), appliedDiscount.code] }), _jsxs("span", { children: ["\u2212", fmt(appliedDiscount.discountAmount)] })] }), _jsxs("div", { className: "flex justify-between px-3 py-2 font-bold text-primary", children: [_jsx("span", { children: "Total" }), _jsx("span", { className: "font-mono", children: fmt(total) })] })] }), _jsxs("div", { className: "space-y-1", children: [_jsx(Label, { children: "Discount code" }), _jsxs("div", { className: "flex gap-2", children: [_jsx(Input, { value: discountInput, onChange: e => setDiscountInput(e.target.value.toUpperCase()), placeholder: "SAVE10", className: "uppercase" }), _jsx(Button, { type: "button", variant: "outline", onClick: applyDiscount, disabled: !discountInput, children: "Apply" })] }), appliedDiscount && _jsxs("p", { className: "text-xs text-green-600 flex items-center gap-1", children: [_jsx(CheckCircle2, { className: "h-3 w-3" }), "Discount applied \u2014 saving ", fmt(appliedDiscount.discountAmount)] })] }), _jsxs("div", { className: "space-y-1", children: [_jsx(Label, { htmlFor: "ship-addr", children: "Shipping address *" }), _jsx(Textarea, { id: "ship-addr", rows: 3, ...register("shippingAddress", { required: true }), placeholder: "123 Main St, City, Country" })] }), _jsxs("div", { className: "space-y-1", children: [_jsx(Label, { htmlFor: "order-notes", children: "Order notes" }), _jsx(Input, { id: "order-notes", ...register("notes"), placeholder: "Special requirements\u2026" })] }), _jsxs(DialogFooter, { children: [_jsx(Button, { variant: "outline", type: "button", onClick: () => setShowCheckout(false), children: "Back to shopping" }), _jsxs(Button, { type: "submit", disabled: placeOrder.isPending || cart.length === 0, children: ["Place order \u2014 ", fmt(total)] })] })] })] }) })] }));
}
/* ═══════════════════════════════════════════════════════════════════════════ */
/*  TAB 2 — MY ORDERS                                                          */
/* ═══════════════════════════════════════════════════════════════════════════ */
