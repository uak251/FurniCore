...
vi.mock("@/components/images/useRecordImages", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@/components/images/useRecordImages")>();
  return {
    ...actual,
...
import type { RecordImage } from "@/components/images/useRecordImages";

const sample: RecordImage[] = [
  {
    id: 1,
...
describe("ModuleGallery", () => {
  it("shows skeleton grid while loading", () => {
    const { container } = render(
      <ModuleGallery entityType="inventory" images={[]} isLoading canUpload={false} />,
    );
  ...
  it("renders raw material label and image preview for inventory images", () => {
    ...
    const img = within(group as HTMLElement).getByRole("img", { name: /a\.jpg/i });
