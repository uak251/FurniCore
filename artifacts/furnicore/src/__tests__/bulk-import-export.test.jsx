...
describe("BulkImportExport", () => {
    ...
    it("clears the preview table after a successful import and shows the result summary", async () => {
        ...
        const fetchMock = vi.fn().mockResolvedValue({
            ok: true,
            text: async () => JSON.stringify({ imported: 0, updated: 1, skipped: 0, errors: [] }),
        });
        globalThis.fetch = fetchMock;
        render(_jsx(BulkImportExport, { module: "Inventory", importEndpoint: "/api/bulk/inventory/import", exportEndpoint: "/api/bulk/inventory/export", exportFilename: "inv.csv", templateHeaders: ["name", "type", "unit", "quantity", "reorderlevel", "unitcost"], templateSample: [["Sample", "raw_material", "kg", "0", "0", "0"]] }));
        ...
        expect(fetchMock).toHaveBeenCalledWith(expect.stringContaining("/api/bulk/inventory/import"), expect.objectContaining({
            method: "POST",
