...
interface FormatOptions {
  /** Show compact notation: 1500 → "1.5K", 1_500_000 → "1.5M" */
  compact?: boolean;
  /** Hide the currency symbol entirely */
  plain?: boolean;
  /** Override decimal places (default: 2, JPY: 0) */
  decimals?: number;
}
...
interface CurrencyContextValue {
  ...
  format:      (amount: number, opts?: FormatOptions) => string;
}
...
export function CurrencyProvider({ children }: { children: ReactNode }) {
  ...
  const format = useCallback(
    (amount: number, opts: FormatOptions = {}): string => {
