...
describe("GET /api/inventory/valuation", () => {
  ...
  it.each([
    ...
    ["LIFO", 150],
    ["WAC", 150],
  ] as const)("returns method %s and totalValue from quantity × unitCost (report line values)", async (method, expectedTotal) => {
    vi.mocked(db.select)
      .mockReturnValueOnce(
        makeChain([{ key: "INVENTORY_VALUATION_METHOD", value: method }]),
      )
      .mockReturnValueOnce(
        makeChain([
          {
