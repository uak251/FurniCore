...
import { emitLowStockAlert } from "./socket";
const STOCK_ALERT_ROLES = ["admin", "manager", "inventory_manager"];
export async function notifyLowStockStakeholders(item) {
    emitLowStockAlert(item);
    try {
        const recipients = await db
            .select({ id: usersTable.id })
            .from(usersTable)
            .where(and(eq(usersTable.isActive, true), inArray(usersTable.role, [...STOCK_ALERT_ROLES])));
