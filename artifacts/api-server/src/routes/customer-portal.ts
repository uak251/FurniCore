...
async function enrichOrderForCustomer(order: typeof customerOrdersTable.$inferSelect) {
...
router.get("/customer-portal/profile", ...customerOnly, async (req: AuthRequest, res): Promise<void> => {
  const [user] = await db
...
router.get("/customer-portal/catalog", ...customerOnly, async (_req, res): Promise<void> => {
...
router.get("/customer-portal/validate-discount", ...customerOnly, async (req, res): Promise<void> => {
  ...
  if (!disc) { res.json({valid: false, reason:"Code not found or inactive"}); return; }
  if (disc.expiresAt && new Date(disc.expiresAt) < new Date()) {
    res.json({valid: false, reason:"Code has expired"}); return;
  }
  if (disc.maxUses && disc.usedCount >= disc.maxUses) {
    res.json({valid: false, reason:"Code has reached its usage limit"}); return;
  }
  if (disc.minOrderAmount && order < Number(disc.minOrderAmount)) {
    res.json({valid: false, reason:`Minimum order amount is $${Number(disc.minOrderAmount).toFixed(2)}`}); return;
  }
...
router.get("/customer-portal/orders", ...customerOnly, async (req: AuthRequest, res): Promise<void> => {
...
router.get("/customer-portal/orders/:id", ...customerOnly, async (req: AuthRequest, res): Promise<void> => {
...
router.post("/customer-portal/orders", ...customerOnly, async (req: AuthRequest, res): Promise<void> => {
  ...
  const products = await Promise.all(
    d.items.map(i => db.select().from(productsTable)
      .where(and(eq(productsTable.id, i.productId), eq(productsTable.isActive, true)))
      .then(r => r[0])),
  );
  if (products.some(p => !p)) { res.status(400).json({error:"One or more products are unavailable"}); return; }
  ...
  await Promise.all(lines.map(({ product, item, unitPrice, lineTotal }) =>
...
router.get("/customer-portal/invoices", ...customerOnly, async (req: AuthRequest, res): Promise<void> => {
...
router.post("/customer-portal/invoices/:id/pay", ...customerOnly, async (req: AuthRequest, res): Promise<void> => {
  const id = parseInt(req.params.id, 10);
  if (isNaN(id)) { res.status(400).json({error:"Invalid id"}); return; }
  ...
  if (inv.status === "paid") { res.status(400).json({error:"Invoice is already paid"}); return; }
