...
function balanceSql(alias: string = "balance") {
  return sql<string>`
    CASE
...
router.get("/reports/trial-balance", authenticate, readRoles, async (req, res, next: NextFunction): Promise<void> => {
    ...
    const rows = await db
      ...
      .leftJoin(
        ...
        and(
          ...
          sql`${journalEntriesTable.date} <= ${asOf}`,
        )
      )
...
router.get("/reports/profit-loss", authenticate, readRoles, async (req, res, next: NextFunction): Promise<void> => {
    ...
    const rows = await db
      ...
      .leftJoin(
        ...
        and(
          eq(journalEntriesTable.id, journalEntryLinesTable.journalEntryId),
          eq(journalEntriesTable.status, "posted"),
          sql`${journalEntriesTable.date} >= ${fromDate}`,
          sql`${journalEntriesTable.date} <= ${toDate}`,
        )
      )
      .where(
        and(
          eq(chartOfAccountsTable.isActive, true),
          sql`${chartOfAccountsTable.type} IN ('income', 'expense')`,
        )
      )
      .groupBy(
        chartOfAccountsTable.id,
...
router.get("/reports/balance-sheet", authenticate, readRoles, async (req, res, next: NextFunction): Promise<void> => {
  const asOf = (req.query.asOf as string) || new Date().toISOString().split("T")[0];
  try {
    const rows = await db
      .select({
      ...
      .where(
        and(
          eq(chartOfAccountsTable.isActive, true),
          sql`${chartOfAccountsTable.type} IN ('asset', 'liability', 'equity')`,
        )
      )
      .groupBy(
        chartOfAccountsTable.id,
...
router.get("/reports/cash-book", authenticate, readRoles, async (req, res, next: NextFunction): Promise<void> => {
  const { from, to } = req.query as { from?: string; to?: string };
  try {
    let query = db
      .select({
    ...
    if (from) query = query.where(sql`${transactionsTable.transactionDate} >= ${from}`) as typeof query;
    if (to)   query = query.where(sql`${transactionsTable.transactionDate} <= ${to + "T23:59:59"}`) as typeof query;
