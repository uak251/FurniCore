...
function penaltiesFromAttendance(
...
router.patch("/attendance/:id", authenticate, async (req: AuthRequest, res): Promise<void> => {
  ...
  if (parsed.data.hoursWorked !== undefined) patch.hoursWorked = String(parsed.data.hoursWorked);
...
router.get("/hr/attendance-summary", authenticate, async (req, res): Promise<void> => {
  ...
  const byEmployee: Record<number, {
    ...
    baseSalary: number; records: { status: string }[];
  }> = {};
...
router.get("/performance-reviews", authenticate, async (req, res): Promise<void> => {
...
router.post("/performance-reviews", ...mgmt, async (req: AuthRequest, res): Promise<void> => {
...
router.patch("/performance-reviews/:id", ...mgmt, async (req: AuthRequest, res): Promise<void> => {
  const id = parseInt(req.params.id, 10);
  if (isNaN(id)) { res.status(400).json({ error: "Invalid id" }); return; }
  ...
  if (d.overallRating       !== undefined) patch.overallRating       = d.overallRating;
  if (d.kpiScore            !== undefined) patch.kpiScore            = String(d.kpiScore);
  if (d.attendanceScore     !== undefined) patch.attendanceScore     = String(d.attendanceScore);
  if (d.punctualityScore    !== undefined) patch.punctualityScore    = String(d.punctualityScore);
  if (d.summary             !== undefined) patch.summary             = d.summary;
  if (d.goals               !== undefined) patch.goals               = d.goals;
  if (d.achievements        !== undefined) patch.achievements        = d.achievements;
  if (d.areasForImprovement !== undefined) patch.areasForImprovement = d.areasForImprovement;
  if (d.recommendBonus      !== undefined) patch.recommendBonus      = d.recommendBonus;
  if (d.bonusSuggestion     !== undefined) patch.bonusSuggestion     = String(d.bonusSuggestion);
...
router.get("/payroll-adjustments", authenticate, async (req, res): Promise<void> => {
  const { employeeId, month, year } = req.query;
  let rows = await db
    .select({
  ...
  if (employeeId) rows = rows.filter((r) => r.adj.employeeId === Number(employeeId));
  if (month)      rows = rows.filter((r) => r.adj.month      === Number(month));
  if (year)       rows = rows.filter((r) => r.adj.year       === Number(year));
...
router.post("/payroll-adjustments", ...mgmt, async (req: AuthRequest, res): Promise<void> => {
...
router.post("/payroll/:id/regenerate", ...mgmt, async (req: AuthRequest, res): Promise<void> => {
  const id = parseInt(req.params.id, 10);
  if (isNaN(id)) { res.status(400).json({ error: "Invalid id" }); return; }
  ...
  const adjustments = await db
    ...
    .where(and(
      ...
      eq(payrollAdjustmentsTable.month, record.month),
      eq(payrollAdjustmentsTable.year,  record.year),
    ));
