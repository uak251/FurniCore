...
router.get("/users/:id/permissions", authenticate, requireRole("admin"), async (req, res, next) => {
...
router.patch("/users/:id/permissions", authenticate, requireRole("admin"), async (req, res, next) => {
    const id = parseInt(req.params.id, 10);
    if (isNaN(id)) {
        res.status(400).json({ error: "Invalid id" });
        return;
    ...
    if (!Array.isArray(permissions)) {
        res.status(400).json({ error: "permissions must be an array of module strings" });
        return;
    }
    try {
