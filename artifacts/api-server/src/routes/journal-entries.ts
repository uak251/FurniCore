...
import { db, journalEntriesTable, journalEntryLinesTable, chartOfAccountsTable, transactionsTable } from "@workspace/db";
...
async function getEntryWithLines(id: number) {
...
/* ── GET /journal-entries ────────────────────────────────────────────────────*/
router.get("/journal-entries", authenticate, async (req, res, next: NextFunction): Promise<void> => {
  try {
    const { status, from, to, limit = "50", offset = "0" } = req.query as Record<string, string>;
    let query = db.select({
      ...
      totalDebit:   sql<string>`COALESCE((SELECT SUM(debit)  FROM journal_entry_lines WHERE journal_entry_id = journal_entries.id), 0)`,
      totalCredit:  sql<string>`COALESCE((SELECT SUM(credit) FROM journal_entry_lines WHERE journal_entry_id = journal_entries.id), 0)`,
    })
    .from(journalEntriesTable)
    .$dynamic();

    if (status)                   query = query.where(eq(journalEntriesTable.status, status)) as typeof query;
...
router.post("/journal-entries", authenticate, requireRole("admin", "accountant"), async (req: AuthRequest, res, next: NextFunction): Promise<void> => {
  const { date, description, referenceType, referenceId, notes, lines, autoPost } = req.body as {
    date: string;
    description?: string;
  ...
  if (!date) { res.status(400).json({ error: "VALIDATION_ERROR", message: "date is required." }); return; }
...
router.get("/journal-entries/:id", authenticate, async (req, res, next: NextFunction): Promise<void> => {
...
router.patch("/journal-entries/:id", authenticate, requireRole("admin", "accountant"), async (req: AuthRequest, res, next: NextFunction): Promise<void> => {
  const id = parseInt(req.params.id as string, 10);
  if (isNaN(id)) { res.status(400).json({ error: "INVALID_ID" }); return; }
  try {
    ...
    const { date, description, notes, lines } = req.body as any;
    const updates: any = {};
    ...
    if (Array.isArray(lines)) {
      ...
      await db.insert(journalEntryLinesTable).values(
        lines.map((l: any) => ({ journalEntryId: id, accountId: l.accountId, description: l.description, debit: String(l.debit ?? 0), credit: String(l.credit ?? 0) }))
      );
    }
...
router.post("/journal-entries/:id/post", authenticate, requireRole("admin", "accountant"), async (req: AuthRequest, res, next: NextFunction): Promise<void> => {
  const id = parseInt(req.params.id as string, 10);
  if (isNaN(id)) { res.status(400).json({ error: "INVALID_ID" }); return; }
  try {
...
router.post("/journal-entries/:id/reverse", authenticate, requireRole("admin"), async (req: AuthRequest, res, next: NextFunction): Promise<void> => {
  const id = parseInt(req.params.id as string, 10);
  if (isNaN(id)) { res.status(400).json({ error: "INVALID_ID" }); return; }
  try {
    const original = await getEntryWithLines(id);
    if (!original) { res.status(404).json({ error: "NOT_FOUND" }); return; }
