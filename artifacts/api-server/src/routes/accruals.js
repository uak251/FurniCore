...
async function getAccrualAccounts(type, overrideAccountId) {
...
/* ── GET /accruals ───────────────────────────────────────────────────────────*/
router.get("/accruals", authenticate, async (req, res, next) => {
    const { status, type, relatedEntityType, relatedEntityId } = req.query;
    try {
        let query = db
            .select({
            id: accrualsTable.id,
            ...
            .$dynamic();
        if (status)
            query = query.where(eq(accrualsTable.status, status));
        if (type)
            query = query.where(eq(accrualsTable.type, type));
        if (relatedEntityType)
            query = query.where(eq(accrualsTable.relatedEntityType, relatedEntityType));
        if (relatedEntityId)
            query = query.where(eq(accrualsTable.relatedEntityId, parseInt(relatedEntityId, 10)));
...
/* ── POST /accruals ─────────────────────────────────────────────────────────*/
router.post("/accruals", authenticate, requireRole("admin", "accountant"), async (req, res, next) => {
    const { type, description, amount, accrualDate, accountId, counterAccountId, relatedEntityType, relatedEntityId, notes, recognitionDate } = req.body;
...
/* ── GET /accruals/:id ───────────────────────────────────────────────────────*/
router.get("/accruals/:id", authenticate, async (req, res, next) => {
...
/* ── PATCH /accruals/:id ─────────────────────────────────────────────────────*/
router.patch("/accruals/:id", authenticate, requireRole("admin", "accountant"), async (req, res, next) => {
    const id = parseInt(req.params.id, 10);
    if (isNaN(id)) {
        res.status(400).json({ error: "INVALID_ID" });
        return;
    ...
    try {
        ...
        const { description, notes, recognitionDate } = req.body;
        const updates = {};
        ...
        if (recognitionDate !== undefined)
            updates.recognitionDate = recognitionDate;
...
router.post("/accruals/:id/recognize", authenticate, requireRole("admin", "accountant"), async (req, res, next) => {
    const id = parseInt(req.params.id, 10);
    if (isNaN(id)) {
        res.status(400).json({ error: "INVALID_ID" });
        return;
    ...
    try {
        ...
        if (!accrual.accountId || !accrual.counterAccountId) {
            res.status(422).json({ error: "MISSING_ACCOUNTS" });
            return;
        }
        const recognitionDate = req.body.date ?? new Date().toISOString().split("T")[0];
...
router.post("/accruals/:id/reverse", authenticate, requireRole("admin"), async (req, res, next) => {
    const id = parseInt(req.params.id, 10);
    if (isNaN(id)) {
        res.status(400).json({ error: "INVALID_ID" });
        return;
    ...
    try {
        ...
        if (accrual.status === "reversed") {
            res.status(400).json({ error: "ALREADY_REVERSED" });
            return;
        }
        if (!accrual.accountId || !accrual.counterAccountId) {
            res.status(422).json({ error: "MISSING_ACCOUNTS" });
...
router.get("/sales-manager/receivables-accruals", authenticate, requireRole("admin", "accountant", "manager", "sales_manager"), async (req, res, next) => {
    const { customerId } = req.query;
    try {
        let q = db.select().from(accrualsTable).where(and(eq(accrualsTable.relatedEntityType, "customer"), eq(accrualsTable.type, "accrued_income"))).$dynamic();
        if (customerId)
            q = q.where(eq(accrualsTable.relatedEntityId, parseInt(customerId, 10)));
...
/** GET /suppliers/:id/accruals — payable accruals for a supplier */
router.get("/suppliers/:id/accruals", authenticate, requireRole("admin", "accountant", "manager"), async (req, res, next) => {
    ...
    try {
        const rows = await db.select().from(accrualsTable).where(and(eq(accrualsTable.relatedEntityType, "supplier"), eq(accrualsTable.relatedEntityId, supplierId))).orderBy(desc(accrualsTable.accrualDate));
        res.json(rows.map((r) => ({ ...r, amount: Number(r.amount) })));
    }
    catch (err) {
        next(err);
