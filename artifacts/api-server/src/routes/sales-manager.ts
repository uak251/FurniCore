...
async function enrichOrder(order: typeof customerOrdersTable.$inferSelect) {
...
router.get("/sales-manager/overview", ...salesAuth, async (_req, res): Promise<void> => {
  const now   = new Date();
...
router.get("/sales-manager/orders", ...salesAuth, async (_req, res): Promise<void> => {
  const orders = await db.select().from(customerOrdersTable).orderBy(desc(customerOrdersTable.createdAt));
  const enriched = await Promise.all(orders.map(enrichOrder));
  res.json(enriched);
});

router.get("/sales-manager/orders/:id", ...salesAuth, async (req, res): Promise<void> => {
...
router.post("/sales-manager/orders", ...salesAuth, async (req: AuthRequest, res): Promise<void> => {
  ...
  const products = await Promise.all(
    d.items.map(i => db.select().from(productsTable).where(eq(productsTable.id, i.productId)).then(r => r[0])),
  );
  if (products.some(p => !p)) { res.status(400).json({error:"One or more products not found"}); return; }
  ...
  if (d.discountCode) {
    const [disc] = await db.select().from(discountsTable).where(
      and(eq(discountsTable.code, d.discountCode), eq(discountsTable.isActive, true)),
    );
  ...
  // Insert line items
  await Promise.all(lines.map(({ product, item, unitPrice, discPct, lineTotal }) =>
...
router.patch("/sales-manager/orders/:id", ...salesAuth, async (req: AuthRequest, res): Promise<void> => {
  const id = parseInt(req.params.id, 10);
  if (isNaN(id)) { res.status(400).json({error:"Invalid id"}); return; }
  ...
  if (d.shippingAddress   !== undefined) patch.shippingAddress   = d.shippingAddress;
  if (d.estimatedDelivery !== undefined) patch.estimatedDelivery = d.estimatedDelivery ? new Date(d.estimatedDelivery) : null;
  if (d.taskId            !== undefined) patch.taskId            = d.taskId;
...
router.post("/sales-manager/orders/:id/updates", ...salesAuth, async (req: AuthRequest, res): Promise<void> => {
  const id = parseInt(req.params.id, 10);
  if (isNaN(id)) { res.status(400).json({error:"Invalid id"}); return; }
...
router.get("/sales-manager/invoices", ...salesAuth, async (_req, res): Promise<void> => {
...
router.post("/sales-manager/invoices", ...salesAuth, async (req: AuthRequest, res): Promise<void> => {
  const body = z.object({
...
router.patch("/sales-manager/invoices/:id", ...salesAuth, async (req: AuthRequest, res): Promise<void> => {
  const id = parseInt(req.params.id, 10);
  if (isNaN(id)) { res.status(400).json({error:"Invalid id"}); return; }
  ...
  if (d.paymentMethod    !== undefined) patch.paymentMethod    = d.paymentMethod;
  if (d.paymentReference !== undefined) patch.paymentReference = d.paymentReference;
  if (d.notes            !== undefined) patch.notes            = d.notes;
  if (d.status === "paid") patch.paidAt = new Date();
...
router.get("/sales-manager/discounts", ...salesAuth, async (_req, res): Promise<void> => {
...
router.post("/sales-manager/discounts", ...salesAuth, async (req: AuthRequest, res): Promise<void> => {
...
router.patch("/sales-manager/discounts/:id", ...salesAuth, async (req, res): Promise<void> => {
  const id = parseInt(req.params.id, 10);
  if (isNaN(id)) { res.status(400).json({error:"Invalid id"}); return; }
  ...
  const patch: Record<string,unknown> = {};
  const d = parsed.data;
  if (d.code           !== undefined) patch.code           = d.code.toUpperCase();
  if (d.description    !== undefined) patch.description    = d.description;
  if (d.type           !== undefined) patch.type           = d.type;
  if (d.value          !== undefined) patch.value          = String(d.value);
  if (d.minOrderAmount !== undefined) patch.minOrderAmount = String(d.minOrderAmount);
...
router.delete("/sales-manager/discounts/:id", ...salesAuth, async (req, res): Promise<void> => {
  const id = parseInt(req.params.id, 10);
  if (isNaN(id)) { res.status(400).json({error:"Invalid id"}); return; }
...
router.get("/sales-manager/receivables", ...salesAuth, async (_req, res): Promise<void> => {
  const now = new Date();
  ...
  const rows = invoices.map(inv => {
    ...
    if (inv.dueDate) {
      ...
      if (ageDays > 90) { bucket = "over90"; buckets.over90 += amount; }
      else if (ageDays > 60) { bucket = "days90"; buckets.days90 += amount; }
      else if (ageDays > 30) { bucket = "days60"; buckets.days60 += amount; }
      else if (ageDays > 0)  { bucket = "days30"; buckets.days30 += amount; }
