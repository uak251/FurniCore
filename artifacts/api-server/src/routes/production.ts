...
async function enrichOrder(o: typeof productionOrdersTable.$inferSelect) {
...
async function enrichQcRemark(r: typeof qcRemarksTable.$inferSelect) {
...
async function enrichMaterial(m: typeof materialUsageTable.$inferSelect) {
...
const OrderStatuses = ["planned", "in_production", "quality_check", "completed", "cancelled"] as const;
...
const QcResults = ["pass", "fail", "hold"] as const;
...
router.get("/production-orders", authenticate, async (_req, res): Promise<void> => {
  const rows = await db
    .select()
 ...
 */
router.get("/qc-remarks/public", async (req, res): Promise<void> => {
  const taskId = req.query.taskId ? Number(req.query.taskId) : null;
...
router.patch(
  ...
  async (req: AuthRequest, res): Promise<void> => {
    ...
    if (!remark) { res.status(404).json({ error: "QC remark not found" }); return; }
...
router.get("/material-usage", authenticate, async (req, res): Promise<void> => {
  const taskId = req.query.taskId ? Number(req.query.taskId) : null;
...
router.post("/material-usage", authenticate, async (req: AuthRequest, res): Promise<void> => {
...
router.delete(
  ...
  async (req: AuthRequest, res): Promise<void> => {
    ...
    if (!usage) { res.status(404).json({ error: "Material usage record not found" }); return; }
    res.sendStatus(204);
  },
);
