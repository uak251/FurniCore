...
async function toTask(t: typeof manufacturingTasksTable.$inferSelect) {
...
router.get("/manufacturing", authenticate, async (req, res): Promise<void> => {
...
router.post("/manufacturing", authenticate, async (req: AuthRequest, res): Promise<void> => {
...
router.get("/manufacturing/:id", authenticate, async (req, res): Promise<void> => {
...
router.patch("/manufacturing/:id", authenticate, async (req: AuthRequest, res): Promise<void> => {
  ...
  if (parsed.data.estimatedHours !== undefined) updateData.estimatedHours = String(parsed.data.estimatedHours);
  if (parsed.data.actualHours !== undefined) updateData.actualHours = String(parsed.data.actualHours);
  if (parsed.data.dueDate !== undefined) updateData.dueDate = new Date(parsed.data.dueDate);
  if (parsed.data.status === "completed" && !old.completedAt) updateData.completedAt = new Date();
...
router.delete("/manufacturing/:id", authenticate, async (req: AuthRequest, res): Promise<void> => {
